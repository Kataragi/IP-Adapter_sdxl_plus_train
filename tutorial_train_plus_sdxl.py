...
def main():
    ...
    image_proj_model = Resampler(
        dim=1280 #多分unet.config.cross_attention_dim（おそらく2048。二つのテキストエンコーダーの結合）のままだと動かない？
        depth=4,
        dim_head=64,
        heads=20, #dimが増えたのでヘッド数が増えた
        num_queries=args.num_tokens,
        embedding_dim=1280 #多分image_encoder.config.hidden_sizeのままでも動く？
        output_dim=2048 #多分unet.config.cross_attention_dimのままでも動く？
        ff_mult=4
    )
    ### 変更箇所 ###
